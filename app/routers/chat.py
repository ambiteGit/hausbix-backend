...
from ..push import enviar_push, enviar_web_push
...
@router.get("/no-leidos-total")
def total_mensajes_no_leidos(db: Session = Depends(get_db), usuario: models.Usuario = Depends(usuario_actual)):
    """
    Solo el número total de mensajes sin leer, de todas las conversaciones
    a la vez — para el avisador del icono de mensajes, sin tener que pedir
    el listado completo de conversaciones solo para eso.
    """
    total = (
        db.query(func.count(models.Mensaje.id))
        .join(models.Conversacion, models.Mensaje.conversacion_id == models.Conversacion.id)
        .filter(
            or_(models.Conversacion.comprador_id == usuario.id, models.Conversacion.vendedor_id == usuario.id),
...
def enviar_mensaje(
    ...
    if destinatario:
        ...
        suscripciones_web = db.query(models.SuscripcionWebPush).filter_by(usuario_id=destinatario.id).all()
        for suscripcion in suscripciones_web:
            valida = enviar_web_push(
                suscripcion,
                f"Nuevo mensaje de {usuario.nombre}",
                payload.texto.strip()[:100],
                data={"conversacionId": conversacion_id, "url": f"/chat.html?id={conversacion_id}"},
            )
            if not valida:
                db.delete(suscripcion)  # el navegador ya no la reconoce — se limpia para no reintentar en vano
        db.commit()
